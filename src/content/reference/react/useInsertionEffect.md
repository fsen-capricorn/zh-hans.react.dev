...
* `setup`：处理副作用的函数。setup 函数选择性返回一个*清理*（cleanup）函数。当您的组件添加到DOM中，但在任何布局触发之前，React将运行您的setup函数。在每次重新渲染时，如果依赖项发生变化，React首先会使用旧值运行清理函数（如果提供），然后使用新值运行您的setup函数。当您的组件从DOM中移除时，React将运行您的清理函数。
